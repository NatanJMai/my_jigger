...
function initAreaChart() {
  ...
  const endpoint = mainChartContainer.dataset.chartEndpoint ||
    brushChartContainer.dataset.chartEndpoint; // Check both just in case

  if (!endpoint) {
    console.error("Endpoint not found for GitHub-style chart.");
    return;
  }
  ...
  fetch(endpoint)
    ...
    .then(data => {
      ...
      // --- DATA TRANSFORMATION ---
      // Convert separate categories (dates) and series (values) into
      // the time-series format [timestamp, value] expected by ApexCharts.
      const seriesData = data.categories.map((dateStr, index) => {
        // Assuming data.series is an array with one object: [{ name: '...', data: [...] }]
        const value = data.series[0].data[index];

        // ApexCharts works best if the date is converted to a timestamp
        return [new Date(dateStr).getTime(), value];
      });

      const seriesName = data.series[0].name || 'Sales';
      // ---------------------------

      // 2. Initialize the Brush/Overview Chart (#area-chart-github2)
      new CustomApexChart({
        selector: '#area-chart-github2',
        options: () => ({
          chart: {
            id: 'chartyear', // IMPORTANT: This ID links the two charts
            type: 'area',
            height: 200,
            toolbar: {
              show: false,
          ...
          series: [{
            name: seriesName,
            data: seriesData // USE DYNAMIC DATA
          }],
          yaxis: {
            tickAmount: 10,
            labels: {
      ...
      // 3. Initialize the Main Chart (#area-chart-github)
      new CustomApexChart({
        ...
        options: () => ({
          chart: {
            ...
            brush: {
              enabled: true,
              target: 'chartyear' // CRITICAL: Links to the first chart's ID
            },
            selection: {
              enabled: true,
              // Initial selection range (optional, can be removed)
